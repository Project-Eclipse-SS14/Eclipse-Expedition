...
use common::{
    SS14Project,
    prototypes_ftl::{EntityPrototypeId, FtlStorage, StringOrOtherId},
};
...
impl TranslatableAttributes {
    ...
    pub fn try_resolve_from_parent(&mut self, parent_id: &EntityPrototypeId, parent_attrs: &Self) {
        self.name = self.name.take().or_else(|| {
            if parent_attrs.name.is_some() {
                Some(StringOrOtherId::Id(parent_id.clone()))
            } else {
                None
            }
        });
    ...
    pub fn resolve_using_parent_id(&mut self, parent_id: &EntityPrototypeId) {
        self.name = self
            .name
            .take()
            .or_else(|| Some(StringOrOtherId::Id(parent_id.clone())));
...
fn main() {
    ...
    let save_locale_path = PathBuf::from(PROJECT_ROOT_PATH).join(PathBuf::from(SAVE_LOCALE_PATH));
    ...
    for v in project.get_prototypes() {
        ...
        for obj in seq {
            ...
            let id = match obj.get(&Yaml::String("id".to_owned())).unwrap() {
                Yaml::String(v) => EntityPrototypeId::parse_without_strip(v.to_string()),
                k => {
            ...
            let parents = obj
                ...
                .map(|v| match v {
                    Yaml::String(v) => vec![EntityPrototypeId::parse_without_strip(v.clone())],
                    Yaml::Array(v) => v
                        .iter()
                        .map(|v| {
                            EntityPrototypeId::parse_without_strip(v.as_str().unwrap().to_string())
                        })
                        .collect(),
            ...
            id_to_path_map.insert(id.clone(), path.clone());
    ...
    // Remove translated prototypes (in ftls) that are not present in ymls
    ftl_project.files.iter_mut().for_each(|(path, file)| {
        let len = file.prototypes.len();
        file.prototypes.retain(|prototype| {
            let Some(prototype_path) = id_to_path_map.get(&prototype.borrow().id) else {
                return false;
            };
            let prototype_ftl_path = save_locale_path
                .join(pathdiff::diff_paths(prototype_path, project.get_resources_path()).unwrap())
                .with_extension("ftl");
            ftl_project.root_path.join(path) == prototype_ftl_path
        });
        if file.prototypes.len() != len {
            file.mark_as_changed();
        }
    });

    ftl_project.remove_empty_files();
    ...
    for (id, path, mut attributes) in prototypes {
        ...
        let hash: [u8; 32] = {
            ...
            if let Some(v) = &attributes.name {
                match v {
                    ...
                    StringOrOtherId::Id(v) => hasher.update(v.inner_id()),
                };
            }
        ...
        match file.get_prototype(id.as_ref()) {
            Some(prototype) => {
...
fn resolve_using_parents(
    id_map: &HashMap<EntityPrototypeId, TranslatableAttributes>,
    parents_map: &HashMap<EntityPrototypeId, Vec<EntityPrototypeId>>,
    parent_id: &EntityPrototypeId,
